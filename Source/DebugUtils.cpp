...
void DebugUtils::ShowAssertDialogue() {
    ...
    std::wstring msg = L"SSRDecompile version " + version + L" has encountered an error.\n";
    msg += L"Please press Control C to copy this error, and paste it to darkid.\n";
    msg += GetStackTrace();
    MessageBox(NULL, msg.c_str(), L"SSRDecompile encountered an error.", MB_TASKMODAL | MB_ICONHAND | MB_OK | MB_SETFOREGROUND);
}
